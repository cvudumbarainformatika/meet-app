<template>
  ...
  <div class="min-h-screen bg-background flex relative overflow-hidden text-foreground w-full">
    ...
    <template v-else>
      ...
      <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-background relative z-10 w-full px-4 sm:px-8 pt-8 pb-32 custom-scrollbar">
      ...
      <!-- ===== PROFILE CARD — Fixed Bottom Left (DIKUNCI, tidak berubah) ===== -->
      <div class="fixed bottom-6 left-6 z-30 p-3 bg-black/20 flex items-center gap-2.5 rounded-2xl backdrop-blur-sm border border-border/60 shadow-xl">
      ...
      <div class="fixed bottom-6 right-6 z-30 p-2 bg-black/20 flex items-center gap-1 rounded-2xl backdrop-blur-sm border border-border/60 shadow-xl">
        <button
      ...
      <!-- ===== ROOMS POPUP PANEL (Win11 Start Menu Style) ===== -->
      <Transition name="popup">
        <div
          v-if="showRoomsPopup"
          class="fixed bottom-[5.5rem] left-1/2 -translate-x-1/2 z-30 w-[400px] max-w-[92vw] max-h-[70vh] bg-card/95 backdrop-blur-xl border border-border rounded-2xl shadow-2xl shadow-black/40 overflow-hidden flex flex-col"
          @click.stop
        >
          <!-- Popup Header -->
          ...
          <!-- Room List (Win11 style — list dengan hover card) -->
          <div class="flex-1 overflow-y-auto custom-scrollbar">

            <!-- Loading -->
            <div v-if="loadingAdmin && myRooms.length === 0" class="flex flex-col items-center py-10 text-muted-foreground">
            ...
            <!-- Win11 List Style (lebih baik untuk banyak room) -->
          ...
          <div class="px-5 py-3 border-t border-border bg-card/50 shrink-0">
            <button
              ...
              class="w-full flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors py-1 group"
            >
      ...
      <nav class="fixed bottom-6 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1 bg-card/90 backdrop-blur-md border border-border/80 shadow-2xl shadow-black/30 rounded-2xl px-2 py-2">
...
function toggleRoomsPopup() {
  ...
  if (showRoomsPopup.value && myRooms.value.length === 0) {
    fetchAdminData()
  }
}
