<template>
  ...
  <div class="min-h-screen bg-background flex items-center justify-center p-6 relative overflow-hidden">
    ...
    <div class="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-success/20 blur-[120px] animate-blob mix-blend-screen pointer-events-none"></div>
    <div class="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-primary/20 blur-[120px] animate-blob animation-delay-2000 mix-blend-screen pointer-events-none"></div>
    ...
    <div class="w-full max-w-md relative z-10 animate-fade-in-up">
      ...
      <div class="text-center mb-10">
        ...
        <h1 class="text-foreground text-3xl font-extrabold tracking-tight">Buat Akun Gratis</h1>
        <p class="text-muted-foreground text-sm mt-2">Bergabung dengan {{ settingsStore.appName }} sekarang</p>
      </div>
      ...
      <div class="bg-card/50 backdrop-blur-xl border border-border rounded-3xl p-8 shadow-2xl shadow-black/40 relative overflow-hidden">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6 relative z-10">
          <div class="space-y-1">
            <label class="text-muted-foreground text-xs uppercase tracking-widest font-bold ml-1">Nama Lengkap</label>
            <input
              v-model="form.name"
              type="text"
        ...
        <div class="text-center text-muted-foreground text-sm mt-8 relative z-10 pt-6 border-t border-border">
          Sudah punya akun?
          <RouterLink to="/login" class="text-primary hover:text-primary/80 font-semibold ml-1 transition-colors">
            Masuk di sini
          </RouterLink>
        </div>
      </div>
...
import { Video, Loader2, AlertCircle, UserPlus, ArrowLeft } from 'lucide-vue-next'
